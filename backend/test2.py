...
if ret:
    ...
    # Test YOLO raw detections with default 0.25
    results = model(frame, conf=0.25, iou=0.45, imgsz=1280, verbose=False)[0]
    print(f"YOLO conf=0.25 raw detections: {len(results.boxes)}")
    
    # Test YOLO raw detections with 0.1
    results_low = model(frame, conf=0.1, iou=0.45, imgsz=1280, verbose=False)[0]
    print(f"YOLO conf=0.1 raw detections: {len(results_low.boxes)}")
    
    # Test DeepSORT with the low conf ones
    ...
    print(f"DeepSORT tracks total: {len(tracks)}, confirmed: {confirmed}")
